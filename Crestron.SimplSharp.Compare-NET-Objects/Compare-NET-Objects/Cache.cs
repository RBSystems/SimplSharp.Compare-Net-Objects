    ...
    internal static class Cache
    {
...
#endif
                    currentFields = type.GetCType().GetFields(); //Default is public instance and static
        ...
        public static IEnumerable<PropertyInfo> GetPropertyInfo(ComparisonResult result, Type type)
        {
            ...
            if (!result.Config.CompareStaticProperties)
                ...
                if (result.Config.ComparePrivateProperties && !result.Config.CompareStaticProperties)
                    currentProperties =
                        type.GetCType().GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                else if (result.Config.ComparePrivateProperties && result.Config.CompareStaticProperties)
                    currentProperties =
                        type.GetCType().GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic |
                                           BindingFlags.Static);
                else if (!result.Config.CompareStaticProperties)
                    currentProperties = type.GetCType().GetProperties(BindingFlags.Public | BindingFlags.Instance);
                else
                    currentProperties = type.GetCType().GetProperties(); //Default is public instance and static
#endif
        ...
        public static IEnumerable<MethodInfo> GetMethods(Type type)
        {
            lock (_methodList)
            {
                ...
                MethodInfo[] myMethodInfo = type.GetCType().GetMethods();
